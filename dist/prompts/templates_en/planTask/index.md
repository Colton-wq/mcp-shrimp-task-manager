...
7. **Mandatory Quality Subtasks Generation**

- Always include the following subtasks in the initial task breakdown (adjust names to the project):
  - Unit testing subtasks (local runner via Desktop Commander)
  - Component testing subtasks (local runner via Desktop Commander)
  - E2E testing subtasks using DIRECT Playwright MCP tool calls during execution (no manual scripts)
  - Code review subtasks via external code review MCP server; expect structured results with severity
- Ensure these quality subtasks are linked as dependencies for feature delivery tasks.
...
**Now start calling `analyze_task` and then proceed automatically**

Automation Mode:
- After analysis and reflection steps, proceed autonomously; no manual approval required.
- Immediately proceed to task splitting and execution using the appropriate tools.
- Execute tasks continuously in dependency order until all subtasks from the original request are completed and verified.
- Maintain quality checks and best practices at each step.