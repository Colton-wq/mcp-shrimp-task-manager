...
describe('AI Calling Effectiveness', () => {
  ...
  describe('Response Format Consistency', () => {
    ...
    it('should format error responses consistently', () => {
      ...
      expect(text).toMatch(/❌ INTERNAL_ERROR: Database connection failed/);
      expect(text).toMatch(/📋 Details: Connection timeout after 30 seconds/);
      expect(text).toMatch(/🔧 Recovery Action: Check database connectivity and retry/);
      expect(text).toMatch(/🔄 This operation can be retried/);
    });
  });
    ...
    it('should provide appropriate path recommendations based on complexity', () => {
      const testCases = [
        ...
        },
        {
          ...
          expectedAdvice: 'plan_task → execute_task'
        },
        {
          complexity: 'HIGH',
          expectedPath: '🔬 Deep Path',
          expectedAdvice: 'plan_task → analyze_task → reflect_task → split_tasks → execute_task'
        }
      ];
