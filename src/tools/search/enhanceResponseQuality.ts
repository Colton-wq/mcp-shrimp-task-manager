...
/**
 * Enhance Response Quality - MCP-compliant AI behavior intervention tool
 * Analyzes conversation patterns and generates mandatory search strategies
 * for evidence-based AI responses
 */

export const enhanceResponseQualitySchema = z.object({
...
export async function enhanceResponseQuality({
  conversationContext,
  problemDescription,
  currentApproach = "",
  uncertaintyLevel,
  errorCount = 0,
}: z.infer<typeof enhanceResponseQualitySchema>) {
  // 开始跟踪工具调用
  const callId = `enhance-response-quality-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  toolCallTracker.startTracking(callId, "enhance-response-quality", {
    uncertaintyLevel,
    errorCount,
